...
export default {
  ...
  computed: {
    ...mapState(['fetching', 'Account']),
    isAccessDenied() {
      return this.$route.params.accessDenied || false
    },
    ...
    submit() {
      ...
      if (!this.$v.$invalid) {
        ...
        this.attemptSaveUserMetadata(this.createObjMetadata())
          .then(({ data }) => {
            this.Account.user.metaStatus = 'completed'
            this.setFetching(true)
...
<template>
  ...
  >
    <action
      v-if="!forceMetadataFill && !blockMetaOptIn()"
      type="button"
      icon="keyboard_backspace"
      icon-size="medium"
      :text="$t('dashboard.profile:modal.back')"
    ...
    <div class="modal-form modal-form-first-access">
      <h2 class="modal-title text-color">
        {{
          !isAccessDenied
            ? $t('dashboard.profile.pending:modal.title', { name: Account.user.data.name })
            : $t('dashboard.profile.pending.denied:modal.title')
        }}
      </h2>
      <div class="modal-description text-color">
        <p class="text-color">
          {{
            !isAccessDenied
              ? $t('dashboard.profile.pending:modal.description.1')
              : $t('dashboard.profile.pending.denied:modal.description.1')
          }}
        </p>
        <p v-if="!blockMetaOptIn()">{{ $t('dashboard.profile.pending:modal.description.2') }}</p>
      </div>
      <form @submit.prevent="submit">
        <template v-for="(metadata, index) in metadataList">
          ...
          <input-field
            ...
            @input="validateData($event, metadata.alias)"
          ></input-field>
          <input-field
            :label="metadata.name"
            v-model="formData.metadata[metadata.alias]"
