...
const getRootFolder = (folders) => {
  return {
    id: 0,
    name: i18n.t('global:menu.classroom.conference'),
    children: folders
  }
}
...
export const getBreadcrumb = (targetId, folders, keepLast) => {
  ...
  depthFirstSearchFolder({id: 0, name: i18n.t('global:menu.classroom.conference'), children: folders})
  path.reverse()
...
export const searchFolderByName = (search, folders) => {
  ...
  const byNameDepthFirstSearch = (search, folder, currentBreadCrumb) => {
    ...
    if (folder.name.includes(search)) {
      ...
      matchedFolders.push({...folder, label: folder.name, value: folder.id, breadcrumbs, disabled: folder.disabled})
    }
  ...
  for (let folder of folders) {
    byNameDepthFirstSearch(search, folder, [{key: 0, text: i18n.t('global:menu.classroom.conference'), value: getRootFolder(folders)}])
  }
