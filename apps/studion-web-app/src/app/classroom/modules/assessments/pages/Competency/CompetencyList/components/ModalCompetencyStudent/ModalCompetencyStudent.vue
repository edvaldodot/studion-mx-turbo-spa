...
export default {
  ...
  computed: {
    ...
    messageForEmptyResults() {
      const currentFilter = this.pgtrParams.filter

      return this.pgtrParams.query.name || (currentFilter && currentFilter.status.length > 0)
        ? 'classroom.examinations.users.list:modal.title.empty'
        : 'classroom.competencies.users.list:modal.title.empty'
    },
  },
  ...
  created() {
    ...
    const payload = {
      ...
      query: { enrollment_id: this.student.enrollment_id },
    }
...
<template>
  <Modal :active="true" is-page @close="$emit('close')">
    ...
    <div class="center" v-if="pgtrCurrentData.length > 0">
      <Datatable :items="pgtrCurrentData" dark>
        <template slot="thead">
          <tr>
            <th class="th">{{ $t('classroom.assessments.tab.link.2') }}</th>
    ...
    <EmptyMessage v-else>
      <h2>{{ $t(messageForEmptyResults) }}</h2>
    </EmptyMessage>
  </Modal>
</template>
